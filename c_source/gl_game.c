	...
	;/*
{
	...
	float position_x; 				//-56(%rbp)
	float position_y;				//-64(%rbp)
	hexGridToViewport(iter->position_x, iter->position_y, 
					  state->map_offset_x, state->map_offset_y,
					  state->map_hex_size, 
					  &position_x, &position_y);
	int color = getUnitTeamColor(iter->team, gl_state->colors); //-72(%rbp)
	float color_r = gl_state->colors[color * 3 + 0]; //-76	
	float color_g = gl_state->colors[color * 3 + 1]; //-80
	float color_b = gl_state->colors[color * 3 + 2]; //-84
	float color_a = 1; //-88
	//-96
	sub_texture *temp = 
...
;/*{
	...
	if(state->help == 1)
	{
		updateObjectGL(40, 4, &gl_state->help_object, gl_state);
