...
void initializeGameState(game_state* state)
{
	...
	state->min_offset_x = -5.0;
	state->min_offset_y = -2.0;
	state->max_offset_x = 0.0;
	state->max_offset_y = -0.0;

	state->min_hex_size = 0.05;
	state->max_hex_size = 0.2;
...
void attackSelectedUnit(struct game_state *state)
{
	...
	if(target != -1)
	{
		unit *target_unit = &state->units[target];
		if(target_unit->team != selected_unit->team &&
		   selected_unit->mp_current>0 &&	
		   state->cursor_distance <= selected_unit->attack_range)
		{
			int damage_mod = 1;
			if(selected_unit->type == UNIT_FIRE_ELEMENTAL && target_unit->type == UNIT_ICE_ELEMENTAL)
				damage_mod = 2;
			if(selected_unit->type == UNIT_WATER_ELEMENTAL && target_unit->type == UNIT_FIRE_ELEMENTAL)
				damage_mod = 2;
			if(selected_unit->type == UNIT_ICE_ELEMENTAL && target_unit->type == UNIT_WATER_ELEMENTAL)
				damage_mod = 2;
			state->units[target].health -= damage_mod * state->units[state->selected_unit].attack_damage;
			if(state->units[target].health <= 0)
			{
				if(target_unit->type == UNIT_WORKSHOP)
				{
					state->win = 1;
					alertMessage("Game Over! Hold ESCAPE to close the game.", 100000, state);
					state->mode = MODE_NORMAL;
					selectUnit(-1, state);
					state->cursor_active = 0;
				}
...
void clampMapOffset(game_state *state)
{
	...
	if(state->map_offset_y > state->max_offset_y) 
		state->map_offset_y = state->max_offset_y;
	if(state->map_offset_x < state->min_offset_x) 
		state->map_offset_x = state->min_offset_x;
...
void processInput(struct input_pressed *input, struct game_state *state)
{
	...
	if(state->win==1)
		return;
	if(state->help = input->key_pressed_H)
		state->help = 1;
	else 
		state->help = 0;
	...
	//FIXME(filip): Obvious camera zoom bug
	state->map_hex_size += 0.01*input->scroll_delta;
	input->scroll_delta = 0;

	state->min_offset_x = -5.0 * (state->map_hex_size / 0.1);	
	state->min_offset_y = -2.5 * (state->map_hex_size / 0.1);	
	clampHexSize(state);
...
void generateTestMap(struct game_state *state)
{
	...
	state->terrain_map[3][3] = TILE_ESSENCE;
	state->terrain_map[1][8] = TILE_ESSENCE;
	state->terrain_map[8][1] = TILE_ESSENCE;
	state->terrain_map[state->size_y - 1 - 3][state->size_x - 1 - 3] = TILE_ESSENCE;
	state->terrain_map[state->size_y - 1 - 1][state->size_y - 1 - 8] = TILE_ESSENCE;
	state->terrain_map[state->size_y - 1 - 8][state->size_y - 1 - 1] = TILE_ESSENCE;
	state->terrain_map[3][state->size_x - 1 - 3] = TILE_FIRE;
	state->terrain_map[5][state->size_x - 1 - 5] = TILE_WATER;
	state->terrain_map[7][state->size_x - 1 - 7] = TILE_ICE;
	...
	createWorkshop(state->size_x - 2, state->size_y - 2, 1, state);
	state->players[1].spawn_tile_x = state->size_x-2;
